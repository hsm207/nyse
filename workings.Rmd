...
```{r}
bloomberg.clean.df %<>%
  dplyr::mutate(year = as.numeric(stringr::str_match(year, "CY1 ([0-9]{4})")[, 2]))
```
...
Save cleaned fundamentals data:
```{r}
write.csv(bloomberg.clean.df, "./data/Bloomberg_Fundamentals_cleaned.csv", row.names = F)
```
